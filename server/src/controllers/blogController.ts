...
export const pullBlogs = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.articles.length === 0 && isFakeDataEnabled()) {
      debugLog(
        "📰 [Fallback] No daily articles found, generating fake articles",
      );
...
export async function pullHourlyBlogs(req: Request, res: Response) {
  ...
  try {
    ...
    if (result.articles.length === 0 && isFakeDataEnabled()) {
      debugLog(
        "📰 [Fallback] No hourly articles found, generating fake articles",
      );
      const fakeResponse = await getFakeBlogResponse(8);
      res.json(fakeResponse);
      return;
...
export async function getRelevantArticlesController(
  req: Request,
  res: Response,
) {
...
export async function getArticleByKeyController(req: Request, res: Response) {
  ...
  try {
    ...
    if (article) {
      ...
      if (totalCount === 0) {
        debugLog(
          "📰 [Fallback] Database empty, returning fake article for key:",
          key,
        );
...
export async function getFeaturedArticleController(
  req: Request,
  res: Response,
) {
  ...
  try {
    ...
    if (article) {
      ...
      debugLog(
        "📰 [Fallback] No featured article found, generating fake featured article",
      );
