...
export async function generateScheduledArticles(writingInterval: number) {
  try {
    ...
    const newArticlesNeeded: number =
      MINIMAL_NUM_DAILY_ARTICLES - result.articles.length;
    ...
    const currentNews: NewsItem[] = await getAllNewsArticlesAfterDate(
    ...
    for (let i = 0; i < newArticlesNeeded; i++) {
      ...
      const currentNewsItem = currentNews.splice(
        randomInt(currentNews.length),
        1,
      )[0];
