...
export async function generateScheduledRecipes(writingInterval: number) {
    try {
        ...
        console.log(`🍳 [generateScheduledRecipes] Current recipes in interval: ${result.length}, needed: ${newRecipesNeeded}`);
        
        if (newRecipesNeeded <= 0) {
            console.log('🍳 [generateScheduledRecipes] Sufficient recipes already exist');
            return;
        }
        ...
        console.log(`🍳 [generateScheduledRecipes] Selected foods: ${foods.join(', ')}`);
        ...
        for (let i = 0; i < newRecipesNeeded; i++) {
            try {
                ...
                if (recipeFoods.length === 0) {
                    console.warn('⚠️ [generateScheduledRecipes] No foods available for recipe generation');
                    break;
                }
                
                console.log(`🍳 [generateScheduledRecipes] Generating recipe ${i + 1}/${newRecipesNeeded} with writer: ${writer.name}, foods: ${recipeFoods.join(', ')}`);
        ...
        console.log('✅ [generateScheduledRecipes] Recipe generation cycle completed');
    } catch (error) {
