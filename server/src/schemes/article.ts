import { Post, createPost, getAllPosts, getPostByKey, updatePost, deletePost } from "../lowdb_complete/lowdb_lib/lowdb_crud"
import { DB_BLOG_POST_FILE } from "../lowdb_complete/lowdb_lib/lowdb_databases";
...
export async function articleTest() {
  ...
  for (const p of INITIAL_POSTS) {
    ...
    const foundPost = await getPostByKey<ArticleScheme>(p.key, DB_BLOG_POST_FILE);
