    ...
    astrologicalData: {
      ...
      planets: [
        ...
        },
        {
          ...
          longitude: Math.random() * 360,
          latitude: Math.random() * 180,
          sign: getRandomElement(ZODIAC_SIGNS),
          isRetrograde: Math.random() > 0.9,
        },
      ],
      retrogrades:
        Math.random() > 0.7 ? ['Mercury', 'Venus'].slice(0, Math.floor(Math.random() * 2) + 1) : [],
      notableAspects: ['Sun trine Moon', 'Mars square Jupiter'].slice(
        0,
...
async function generateMockData(options: {
  ...
  try {
    ...
    const newsCountResult = db.prepare('SELECT COUNT(*) as count FROM news_items').get() as {
      count: number;
    };
    const writerCount = db.prepare('SELECT COUNT(*) as count FROM writers').get() as {
      count: number;
    };
    const featuredCountResult = db
      .prepare('SELECT COUNT(*) as count FROM featured_blog_posts')
      .get() as { count: number };
    const recipeCountResult = db.prepare('SELECT COUNT(*) as count FROM recipes').get() as {
      count: number;
    };
    const foodCount = db.prepare('SELECT COUNT(*) as count FROM foods').get() as { count: number };
    const horoscopeCount = db.prepare('SELECT COUNT(*) as count FROM horoscopes').get() as {
      count: number;
    };
