...
function generateArticle(writer?: Writer): ArticleScheme {
  ...
  return {
    ...
    featuredDate:
      Math.random() > 0.7 ? getRandomDate(pastDate, now) : undefined,
  };
}
...
function generateHoroscope(): Horoscope {
  ...
  return {
    ...
    astrologicalData: {
      ...
      retrogrades:
        Math.random() > 0.7
          ? ["Mercury", "Venus"].slice(0, Math.floor(Math.random() * 2) + 1)
          : [],
...
async function generateMockData(options: {
  ...
  try {
    ...
    const featuredCountResult = db
      ...
      .get() as { count: number };
    const recipeCountResult = db
      .prepare("SELECT COUNT(*) as count FROM recipes")
      .get() as { count: number };
    const foodCount = db
      .prepare("SELECT COUNT(*) as count FROM foods")
      .get() as { count: number };
    const horoscopeCount = db
      .prepare("SELECT COUNT(*) as count FROM horoscopes")
      .get() as { count: number };
