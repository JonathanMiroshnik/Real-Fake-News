...
async function crudTest() {
    ...
    // Create - Add new post
    await createPost(testPost, lowdb_databases_1.DB_BLOG_POST_FILE);
    ...
    if (foundPost) {
        const k = { key: "1" };
        await updatePost(k, lowdb_databases_1.DB_BLOG_POST_FILE);
        console.log('Post updated');
    }
    // Delete - Remove post
    const deleted = await deletePost("1", lowdb_databases_1.DB_BLOG_POST_FILE);
    console.log('Post deleted:', deleted);
}
// Execute main example
// crudTest().catch(console.error);
//# sourceMappingURL=lowdb_crud.js.map