...
window.fetch = (e, t) => (
  (e instanceof Request ? e.method : t?.method || 'GET') !== 'GET' && M.delete(xe(e)),
  bt(e, t)
);
const M = new Map();
function kt(e, t) {
  ...
  if (a?.textContent) {
    ...
    return (
      ...
      a.getAttribute('data-b64') !== null && (r = yt(r)),
...
const L = globalThis.__sveltekit_bkzoac?.base ?? '',
  Tt = globalThis.__sveltekit_bkzoac?.assets ?? L ?? '',
  It = '1770051692810',
...
function Pt() {
  ...
  async function a() {
    clearTimeout(n);
    try {
      const r = await fetch(`${Tt}/_app/version.json`, {
...
function Ue(e) {
  return e instanceof Ee || e instanceof Re ? e.status : 500;
}
function jt(e) {
...
const Dt = new Set(['icon', 'shortcut icon', 'apple-touch-icon']),
  D = We(He) ?? {},
  Y = We(Ye) ?? {},
  C = { url: ze({}), page: ze({}), navigating: ke(null), updated: Pt() };
function Ae(e) {
  D[e] = le();
}
function qt(e, t) {
...
async function ln(e, t, n) {
  (globalThis.__sveltekit_bkzoac?.data && globalThis.__sveltekit_bkzoac.data,
  ...
  (n
    ...
    : (await z({
        type: 'enter',
        url: Le(v.hash ? nn(new URL(location.href)) : location.href),
        replace_state: !0,
      }),
      r()),
    Qt());
    ...
    if (
      ...
      (R = {
        ...
        promise: ut({ ...e, preload: t }).then(
          (n) => (oe.delete(t), n.type === 'loaded' && n.state.error && ye(), n),
        ),
        fork: null,
      }),
      qe)
      ...
      n.fork = n.promise.then((a) => {
        if (n === R && a.type === 'loaded')
          try {
            return qe(() => {
              (Oe.$set(a.props), tt(a.props.page));
            });
          } catch {}
        return null;
      });
...
function se({ url: e, params: t, branch: n, status: a, error: r, route: s, form: o }) {
  ...
  ((e.pathname = _t(e.pathname, i)), (e.search = e.search));
  const c = {
    type: 'loaded',
    state: { url: e, params: t, branch: n, error: r, route: s },
    props: { constructors: Ct(n).map((u) => u.node.component), page: Ne(k) },
  };
...
async function ut({ id: e, invalidating: t, url: n, params: a, route: r, preload: s }) {
  ...
  const g = l.map(async (p, d) => {
    if (!p) return;
    const E = _.branch[d];
    return p[1] === E?.loader && !Mt(u, h, f, w, E.universal?.uses, a)
      ? E
      : ((u = !0),
        Pe({
          loader: p[1],
          ...
          server_data_node: $e(p[0] ? { type: 'skip' } : null, p[0] ? E?.server : void 0),
        }));
  });
...
async function Yt(e) {
  ...
  try {
    const a = (async () => {
      let r =
        (await v.hooks.reroute({ url: new URL(e), fetch: async (s, o) => Kt(s, o, e).promise })) ??
        e;
      if (typeof r == 'string') {
        const s = new URL(e);
...
function Ht(e) {
  return (
    mt(v.hash ? e.hash.replace(/^#/, '').replace(/[?#].+/, '') : e.pathname.slice(L.length)) || '/'
  );
}
function ie(e) {
...
async function z({
  ...
  const u = await fe(t, !1),
    g =
      e === 'enter'
        ? je(_, u, t, e)
        : ft({ url: t, type: e, delta: n?.delta, intent: u, event: h });
  if (!g) {
    (f(), N === c && (N = w));
    return;
  }
  ...
  (l(),
    (J = !0),
    re && g.navigation.type !== 'enter' && C.navigating.set((W.current = g.navigation)));
  let d = u && (await ut(u));
  if (!d) {
    ...
    d = await dt(
      t,
      { id: null },
      await X(new Re(404, 'Not Found', `Not found: ${t.pathname}`), {
        url: t,
        params: {},
        route: { id: null },
      }),
  ...
  if (
    ...
    const b = s ? 0 : 1,
      q = { [B]: (y += b), [G]: (x += b), [Je]: o };
    ((s ? history.replaceState : history.pushState).call(history, q, '', t), s || qt(y, x));
  }
  const E = u && R?.id === u.id ? R.fork : null;
  ((R = null), (d.props.page.state = o));
  let S;
  if (re) {
    const b = (await Promise.all(Array.from(Vt, ($) => $(g.navigation)))).filter(
      ($) => typeof $ == 'function',
    );
    if (b.length > 0) {
      let $ = function () {
    ...
    const q = E && (await E);
    (q ? (S = q.commit()) : (Oe.$set(d.props), tt(d.props.page), (S = gt?.())), (ot = !0));
  ...
  if (Me) {
    ...
    if (U) scrollTo(U.x, U.y);
      ...
      U = { x: pageXOffset + $, y: pageYOffset + q };
    } else scrollTo(0, 0);
  }
...
async function dt(e, t, n, a, r) {
  return e.origin === ce && e.pathname === location.pathname && !rt
...
function un(e, t = {}) {
  return (
    (e = new URL(Le(e))),
    e.origin !== ce ? Promise.reject(new Error('goto: invalid URL')) : ct(e, t, 0)
  );
}
function Xt(e) {
  ...
  ((history.scrollRestoration = 'manual'),
    ...
    O.addEventListener('click', async (t) => {
      ...
      if (f !== void 0 && h) {
        ...
        if (((K = !0), Ae(y), e(a), !i.replace_state)) return;
        K = !1;
      }
      (t.preventDefault(),
        await new Promise((w) => {
...
export { ln as a, L as b, un as g, rn as l, k as p, C as s };
