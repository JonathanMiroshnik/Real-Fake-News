<script lang="ts">
	...
	onMount(() => {
		// DEBUG MODE: Accept any password or no password
		const urlPassword = $page.url.searchParams.get(ADMIN_PASSWORD_PARAM);
		// For debugging, we'll accept any password (even empty) or no password at all
		// In production, you'd want to check for a valid password here
		if (urlPassword !== null) {
			password = urlPassword;
		} else {
			// No password in URL, but for debugging we'll still allow access
			password = 'debug'; // Use a dummy password for the API calls
		}
		isAuthorized = true;
		fetchArticles();
		fetchTexts();
