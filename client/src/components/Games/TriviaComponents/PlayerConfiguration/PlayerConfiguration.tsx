...
function PlayerConfiguration(
  playerConfigurationProps: PlayerConfigurationProps,
) {
  return (
    <div className="trivia-player-configuration-container">
      <h1 className="trivia-player-configuration-title">Trivia Challenge</h1>
      <h2 className="trivia-player-configuration-title-player-number">
        Players: {playerConfigurationProps.players.length}
      </h2>

      <div className="trivia-players-grid">
        {playerConfigurationProps.players.map((player, index) => (
          <div
            key={"player_config_" + player.toString() + "_" + index.toString()}
          >
            <input
              ...
              onChange={(e) => {
                playerConfigurationProps.onPlayerChange(index, {
                  name: e.target.value,
                  score: player.score,
                });
              }}
      ...
      <div className="trivia-controls">
        {playerConfigurationProps.players.length < MAX_NUMBER_PLAYERS && (
          <button
            className="trivia-player-configuration-button"
            onClick={playerConfigurationProps.addPlayer}
          >
            {" "}
            +{" "}
          </button>
        ...
        {playerConfigurationProps.players.length > MIN_NUMBER_PLAYERS && (
          <button
            className="trivia-player-configuration-button"
            onClick={playerConfigurationProps.removePlayer}
          >
            {" "}
            -{" "}
          </button>
