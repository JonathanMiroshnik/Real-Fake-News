...
function TicTacToeGame() {
  ...
  function checkVictory(currentBoard: CellState[][]): VictoryStatus {
    ...
    for (let i = 0; i < BOARD_SIDE_LENGTH; i++) {
      ...
      for (let j = 0; j < BOARD_SIDE_LENGTH; j++) {
        if (
          initialSymbol !== currentBoard[i][j].symbol ||
    ...
    for (let j = 0; j < BOARD_SIDE_LENGTH; j++) {
      if (
        initialSymbol !== currentBoard[BOARD_SIDE_LENGTH - 1 - j][j].symbol ||
  ...
  function checkMovePossible(
    ...
    // If the symbol of the cell is the same as the one that wants to add to it
    if (
      currentBoard[row][col].symbol !== currentMove.symbol &&
      currentBoard[row][col].symbol !== Symbols._
    ) {
      // If the new value is not greater than the previous value of the cell with an opposing symbol,
      //  we do not change the symbol.
  ...
  return (
    <div className="tictac-game-screen">
      <h1 className="tictac-game-title">
        <b>TIC-TAC-שש-בש</b>
      </h1>
